...
<head>
    ...
    <title>Actualizacion de plan</title>
</head>
        ...
        <div class="row">
            <div class="col-md-10 offset-md-1">
                ...
                try {
                    ...
                    if (!isset($_REQUEST["pla_nombre"])) {
                        throw new PDOException("Por favor digite el nombre");
                    }
                    if (!isset($_REQUEST["pla_costo"])) {
                        throw new PDOException("Por favor digite el valor");
                    }
                    $pla_id = $_REQUEST["pla_id"];
                    $pla_nombre = $_REQUEST["pla_nombre"];
                    $pla_costo = $_REQUEST["pla_costo"];
                    
                    $plan = new plan();
                    $plan->setPlaId($pla_id);
                    $plan->setPlaNombre($pla_nombre);
                    $plan->setPlaCosto($pla_costo);
                    
                    $controladorPlan = new controladorPlan();
                    $resultado = $controladorPlan->actualizar($plan);
